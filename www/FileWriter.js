...
FileWriter.prototype.write = function (data, isPendingBlobReadResult) {
  ...
  var isProxySupportBlobNatively =
    cordova.platformId === "windows8" || cordova.platformId === "windows";
  var isBinary;

  if (data instanceof File) {
    turnFileOrBlobIntoArrayBufferOrStringAndCallWrite.call(
      me,
      data,
      supportsBinary
    );
...
function writeBase64EncodedStringInChunks(
  successCallback,
  errorCallback,
  arrayBuffer
) {
  ...
  function writeConvertedChunk(base64EncodedChunk) {
    execChunkedWrite.call(me, wroteChunk, errorCallback, base64EncodedChunk);
  }
  ...
  function calculateCurrentChunk() {
    sizeOfChunk = Math.min(
      chunkSizeBytes,
      arrayBuffer.byteLength - startOfChunk
    );
...
function turnArrayBufferIntoBase64EncodedString(
  successCallback,
  errorCallback,
  arrayBuffer
) {
...
function execChunkedWrite(successCallback, errorCallback, base64EncodedChunk) {
  var me = this;
  exec(successCallback, errorCallback, "File", "write", [
    me.localURL,
    base64EncodedChunk,
    me.position,
    true,
 ...
 */
function turnFileOrBlobIntoArrayBufferOrStringAndCallWrite(
  fileOrBlob,
  supportsBinary
) {
  var me = this;
  ...
  fileReader.onload = function () {
    ...
    FileWriter.prototype.write.call(
      me,
      this.result,
      true /* isPendingBlobReadResult */
    );
  };
  fileReader.onerror = function () {
    // DONE state
